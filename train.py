...
data_name = 'rsicd'  # base name shared by data files

# Model parameters
emb_dim = 512  # dimension of word embeddings
attention_dim = 1024  # dimension of attention linear layers
h_dim = 1024  # dimension of decoder RNN
dropout = 0.5
...
def main():
    ...
    if checkpoint is None:
        ...
        decoder_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, decoder.parameters()),
                                             lr=decoder_lr)
        encoder = Encoder(False, False, False, False)
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        scores, _, _, _ = pack_padded_sequence(
            scores, caplens_sorted, batch_first=True)
        targets, _, _, _ = pack_padded_sequence(
            targets, caplens_sorted, batch_first=True)
        ...
        losses.update(loss.item(), sum(caplens_sorted))
        top5accs.update(top5, sum(caplens_sorted))
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            for j, p in enumerate(preds):
                temp_preds.append(preds[j][:caplens_sorted[j]])  # remove pads
            preds = temp_preds
